...
int NMulti_Runcomp() {
    // File names
    const char* file1_name = "ncgammaNTag_SK5_INCL.mc.root";
    const char* file2_name = "ncgammaNTagSKG4_INCLt2k_new.mc.root";

    // Histogram name
    const char* hist_name = "htagged_n_ncgamma_angle_all";
    ...
    TH1D* hist2 = dynamic_cast<TH1D*>(file2->Get("htruall_tagn_ncgamma_angle_all"));
    if (!hist2) {
    ...
    hist1->SetLineColor(kBlue); // Set color for first histogram
    ...
    legend->AddEntry(hist1, "Run10 - INCL", "l");
    legend->AddEntry(hist2, "Run11 - INCL", "l");
    legend->Draw();
